...
%# @end group
%# @end example
%# @seealso{odeset, ode78, ode23, ode78}
%# @end deftypefn

function varargout = ode78 (fun, trange, init, varargin)

  if (nargin < 3)
    print_usage ();
  end

  order = 7; # we don't use local extrapolation
  solver = "ode78";
  ...
  if (! isnumeric (trange) || ! isvector (trange))
    error ("Octave:invalid-input-arg",
           "ode78: TRANGE must be a numeric vector");
  end

  if (length (trange) < 2)
    error ("Octave:invalid-input-arg",
           "ode78: TRANGE must contain at least 2 elements");
  elseif (trange(1) == trange(2))
    error ("Octave:invalid-input-arg",
           "ode78: invalid time span, TRANGE(1) == TRANGE(2)");
  else
  ...
  if (! isnumeric (init) || ! isvector (init))
    error ("Octave:invalid-input-arg",
           "ode78: INIT must be a numeric vector");
  end
  init = init(:);
  ...
  if (! isa (fun, "function_handle"))
    error ("Octave:invalid-input-arg",
           "ode78: FUN must be a valid function handle");
  end
  ...
  persistent ode78_ignore_options = ...
    {"BDF", "InitialSlope", "Jacobian", "JPattern",
     "MassSingular", "MaxOrder", "MvPattern", "Vectorized"};

  defaults   = rmfield (defaults, ode78_ignore_options);
  classes    = rmfield (classes, ode78_ignore_options);
  attributes = rmfield (attributes, ode78_ignore_options);

  odeopts = odemergeopts ("ode78", odeopts, defaults, classes, attributes);
  ...
  if (! isempty (odeopts.NonNegative))
    if (isempty (odeopts.Mass))
      ...
      warning ("Octave:invalid-input-arg",
               ["ode78: option 'NonNegative' is ignored", ...
  ...
  solution = integrate_adaptive (@runge_kutta_78,
  ...
  if (strcmp (odeopts.Stats, "on"))
    ...
    nfevals   = 13 * solution.cntcycles;      # number of ode evaluations
...
%!demo
%! %# Demonstrate convergence order for ode78
%! tol = 1e-7 ./ 10.^[0:8];
%! for i = 1 : numel (tol)
%!   opt = odeset ("RelTol", tol(i), "AbsTol", realmin);
%!   [t, y] = ode78 (@(t, y) -y, [0, 20], 1, opt);
%!   h(i) = 1 / (numel (t) - 1);
...
%! loglog (h, tol, "-ob",
%!         h, err, "-b",
%!         h, (h/h(end)) .^ 7 .* tol(end), "k--",
%!         h, (h/h(end)) .^ 8 .* tol(end), "k-");
%! axis tight
%! xlabel ("h");
%! ylabel ("err(h)");
%! title ("Convergence plot for ode78");
%! legend ("imposed tolerance", "ode78 (relative) error",
%!         "order 7", "order 8", "location", "northwest");
%!
...
%! warning ("off", "integrate_adaptive:unexpected_termination", "local");
%! opt = odeset ("Events", @fevn, "NormControl", "on");
%! [t, y, vxe, ye, vie] = ode78 (@fpol, [0 10], [2 0], opt);
%! assert ([vie, vxe, ye],
%!         [2.0, 2.496110, -0.830550, -2.677589], 6e-1);
...
%! sol = ode78 (@(x,y) 1, [0 1], 1i);
%! assert (imag (sol.y), ones (size (sol.y)))
%! [x, y] = ode78 (@(x,y) 1, [0 1], 1i);
%! assert (imag (y), ones (size (y)))

%!error ode78 ()
%!error ode78 (1)
%!error ode78 (1,2)
%!error <TRANGE must be a numeric> ode78 (@fpol, {[0 25]}, [3 15 1])
%!error <TRANGE must be a .* vector> ode78 (@fpol, [0 25; 25 0], [3 15 1])
%!error <TRANGE must contain at least 2 elements> ode78 (@fpol, [1], [3 15 1])
%!error <invalid time span> ode78 (@fpol, [1 1], [3 15 1])
%!error <INIT must be a numeric> ode78 (@fpol, [0 25], {[3 15 1]})
%!error <INIT must be a .* vector> ode78 (@fpol, [0 25], [3 15 1; 3 15 1])
%!error <FUN must be a valid function handle> ode78 (1, [0 25], [3 15 1])
